        ...
        <div class="flex justify-between h-16">
            <div class="flex">
                ...
                <div class="shrink-0 flex items-center">
                    <a href="{{ route('dashboard') }}" class="flex items-center gap-2">
                        ...
                        <span class="hidden sm:block text-white font-bold">{{ config('app.name', 'Laravel') }}</span>
                    </a>
                </div>
                ...
                <div class="hidden space-x-8 sm:-my-px sm:ms-10 sm:flex">
                    ...
                    <x-nav-link :href="route('vehicles.index')" :active="request()->routeIs('vehicles.*')">
                        <i class="fa-solid fa-car mr-1"></i> {{ __('Vehículos') }}
                    </x-nav-link>
                    @endif
                </div>
            </div>
            ...
            <div class="hidden sm:flex sm:items-center sm:ms-6">
                <x-dropdown align="right" width="48">
                    <x-slot name="trigger">
                        <button class="inline-flex items-center px-3 py-2 border border-slate-600 text-sm leading-4 font-medium rounded-md text-slate-200 bg-slate-800 hover:text-white focus:outline-none transition ease-in-out duration-150">
                            <div class="flex items-center">
                        <span class="text-xs bg-indigo-600 text-white px-2 py-0.5 rounded-full font-semibold mr-2">
                    ...
                    <x-slot name="content">
                        <x-dropdown-link :href="route('profile.edit')">
                            <i class="fa-solid fa-user mr-1"></i> {{ __('Profile') }}
                        </x-dropdown-link>
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden sm:hidden">
        ...
        <div class="pt-4 pb-1 border-t border-slate-700">
            <div class="px-4">
                ...
                <div class="text-xs text-indigo-400 font-semibold">{{ ucfirst(Auth::user()->role) }}</div>
