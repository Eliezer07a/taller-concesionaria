            ...
            <div class="p-6 bg-slate-800 border border-slate-700 rounded-xl">
                ...
                <form action="{{ route('vehicles.update', $vehicle) }}" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" name="year" min="1900" max="{{ date('Y') + 1 }}" value="{{ old('year', $vehicle->year) }}" required class="block w-full rounded-lg border-slate-600 bg-slate-900 text-white text-sm focus:ring-indigo-500 focus:border-indigo-500">
                    </div>

                    <div>
                        <label class="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1.5">Patente</label>
                        <input type="text" name="plate" maxlength="10" value="{{ old('plate', $vehicle->plate) }}" required class="block w-full rounded-lg border-slate-600 bg-slate-900 text-white text-sm focus:ring-indigo-500 focus:border-indigo-500">
                    </div>

                    <div>
                        <label class="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1.5">Marca</label>
                        <input type="text" name="brand" maxlength="50" value="{{ old('brand', $vehicle->brand) }}" required class="block w-full rounded-lg border-slate-600 bg-slate-900 text-white text-sm focus:ring-indigo-500 focus:border-indigo-500">
                    </div>

                    <div>
                        <label class="block text-xs font-semibold text-slate-400 uppercase tracking-wider mb-1.5">Modelo</label>
                        <input type="text" name="model" maxlength="50" value="{{ old('model', $vehicle->model) }}" required class="block w-full rounded-lg border-slate-600 bg-slate-900 text-white text-sm focus:ring-indigo-500 focus:border-indigo-500">
                    </div>
