<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 dark:text-gray-200 leading-tight">
            <i class="fa-solid fa-car mr-2 text-indigo-500"></i> Editar Vehículo
        </h2>
    </x-slot>
            ...
            <div class="p-6 bg-white dark:bg-gray-800 shadow sm:rounded-lg">
                ...
                <form action="{{ route('vehicles.update', $vehicle) }}" method="POST" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" name="year" min="1900" max="{{ date('Y') + 1 }}" value="{{ old('year', $vehicle->year) }}" required class="mt-1 block w-full rounded-md border-gray-300 dark:border-slate-600 dark:bg-gray-900 dark:text-white">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Patente</label>
                        <input type="text" name="plate" maxlength="10" value="{{ old('plate', $vehicle->plate) }}" required class="mt-1 block w-full rounded-md border-gray-300 dark:border-slate-600 dark:bg-gray-900 dark:text-white">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">VIN</label>
                        <input type="text" name="vin" maxlength="17" value="{{ old('vin', $vehicle->vin) }}" required class="mt-1 block w-full rounded-md border-gray-300 dark:border-slate-600 dark:bg-gray-900 dark:text-white">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Marca</label>
                        <input type="text" name="brand" maxlength="50" value="{{ old('brand', $vehicle->brand) }}" required class="mt-1 block w-full rounded-md border-gray-300 dark:border-slate-600 dark:bg-gray-900 dark:text-white">
                    </div>

                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-gray-300">Modelo</label>
                        <input type="text" name="model" maxlength="50" value="{{ old('model', $vehicle->model) }}" required class="mt-1 block w-full rounded-md border-gray-300 dark:border-slate-600 dark:bg-gray-900 dark:text-white">
                    </div>
